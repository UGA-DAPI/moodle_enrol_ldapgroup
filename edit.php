...
/**
 * Adds new instance of enrol_ldapgroup to specified course.
 *
...
require('../../config.php');
require_once("$CFG->dirroot/enrol/ldapgroup/edit_form.php");
require_once("$CFG->dirroot/enrol/ldapgroup/locallib.php");
...
require_capability('enrol/ldapgroup:config', $context);

$PAGE->set_url('/enrol/ldapgroup/edit.php', array('courseid'=>$course->id, 'id'=>$instanceid));
...
if ($instanceid) {
    $instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'ldapgroup', 'id'=>$instanceid), '*', MUST_EXIST);
...
$mform = new enrol_ldapgroup_edit_form(null, array($instance, $enrol, $course));
...
if ($mform->is_cancelled()) {
    ...
    if ($data->id) {
        // NOTE: no ldapgroup changes here!!!
        if ($data->roleid != $instance->roleid) {
            // The sync script can only add roles, for perf reasons it does not modify them.
            role_unassign_all(array('contextid'=>$context->id, 'roleid'=>$instance->roleid, 'component'=>'enrol_ldapgroup', 'itemid'=>$instance->id));
        }
...
$PAGE->set_title(get_string('pluginname', 'enrol_ldapgroup'));
